...
var result = {
  ...
  findFake: function(id) {
    if(id){
      ...
      // console.log(cats.cats[id])
      return this.cats[id]
      // for(let i = 0, len = cats.cats.length; i < len; i++){
      //   if
      // }
      // return cats.cats[id];
    }
...
// getRandom = (max, min) => Math.floor(Math.random() * (max - min + 1)) + min;
...
//   const min = 500;
//   for(let i = 1; i < 201; i++){
//     let width = getRandom(max, min);
//     let height = getRandom(max, min);
//     cats[i] = {
//       id: i,
//       key: i,
//       name: faker.name.firstName(),
//       created_at: faker.date.past(),
//       urls: { 
//         small: imageUrl[(i - 1) % 15],
//         // small: faker.random.image(),
//         preLoad: imagePreLoadUrl[(i - 1) % 15]
//       },
//       width,
//       height,
//       views: faker.random.number(),
